...
%
%
%%K1 is the number of octaves
%%K2 is the number of voices per octave (the linearly spaced regime has less)
%
%%second layer filters. 
%J_har = getoptions(options,'J_har',1);
%
%clear filt_opt;
%filt_opt.J=J_har;
%filt_opt.filter_format='fourier';
%filt_opt.min_margin=0;
%filt_opt.boundary='per';
%filters = morlet_filter_bank_1d(K1, filt_opt);
%J=length(filters.psi.filter);
%for j=1:J
%filts{2}.psi{j} = filters.psi.filter{j};
%filts{2}.ds(j)=2^(max(0,j-os));
%end
%filts{2}.psi{J+1} = filters.phi.filter;
%filts{2}.ds(J+1)=2^(max(0,J-os));
%
%J_semi = getoptions(options,'J_semi',2);
%clear filt_opt;
%filt_opt.J=J_semi;
%filt_opt.filter_format='fourier';
%filt_opt.min_margin=0;
%filt_opt.boundary='per';
%filters = morlet_filter_bank_1d(K2, filt_opt);
%J=length(filters.psi.filter);
%for j=1:J
%filts{3}.psi{j} = filters.psi.filter{j};
%filts{3}.ds(j)=2^(max(0,j-os));
%end
%filts{3}.psi{J+1} = filters.phi.filter;
%filts{3}.ds(J+1)=2^(max(0,J-os));
%
