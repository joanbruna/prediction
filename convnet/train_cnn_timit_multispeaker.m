...
imdb = imdb_f;
imdb.images.data = cat(4,imdb.images.data,imdb_m.images.data);
clear imdb_m imdb_f
end
...
nchannels = 5;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f2*randn(1,1,filter_num2,nchannels*NFFT, 'single'), ...
                           'biases', zeros(1, nchannels*NFFT, 'single'), ...
                           'stride', 1, ...
                           'pad',0) ;
                       
%net.layers{end+1} = struct('type', 'relu') ;

net.layers{end+1} = struct('type', 'channelPoole', ...
                            'nchannels', nchannels) ;
...
opts.expDir = '/misc/vlgscratch3/LecunGroup/pablo/models/cnn/timit-cnn-multi/';
%opts.expDir = '/tmp/pablo/timit-cnn-test-lr/';
opts.train.batchSize = 10 ;
opts.train.numEpochs = 100;
opts.train.continue = false ;
opts.train.useGpu = true ;
opts.train.learningRate = [0.1*ones(1,10) 0.01*ones(1,30) 0.001*ones(1,200) 0.0001];
...
%
[net,info_sc_init] = cnn_train_audio(net, imdb, imdb, gB, getValid, opts.train) ;
