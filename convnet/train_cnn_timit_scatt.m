...
f2 = 1;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f2*rand(1,1,filter_num2,2*NFFT, 'single'), ...
...
opts.train.validFreq = 0;
opts.train.saveFreq = 5;
opts.train.startSave = 5;
%opts.train.C = 2^(J+3);
opts.train.C = round(2^(J)+J);
...
opts.train.learningRate = [0.1*ones(1,20) 0.01*ones(1,30) 0.001*ones(1,100) 0.0001];
...
[net,info_sc_init] = cnn_train_audio_sc(net, data_f, data_m, gB, getValid, opts.train) ;
