...
Xt1 = compute_spectrum(x,l_win,overlap);
V1 = abs(Xt1).^p;
[F,N] = size(V1);
...
Xt2 = compute_spectrum(x,l_win,overlap);
V2 = abs(Xt2).^p;
[F,N] = size(V2);
...
X1 = compute_spectrum(x1,l_win,overlap);
V1 = abs(X1).^p;
%X2 = cf_stft(x2,l_win,overlap);
X2 = compute_spectrum(x2,l_win,overlap);
V2 = abs(X2).^p;

mix = x1+x2;

%X = cf_stft(mix,l_win,overlap);
X = compute_spectrum(mix,l_win,overlap);
V = abs(X).^p;
[F,N] = size(V);
