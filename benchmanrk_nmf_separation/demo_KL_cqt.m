...
addpath ../external/scatnet/filters/
addpath ../external/scatnet/utils/
filts = cqt_prepare(scparam);


% for testing
scparam_2.T = NFFT;
scparam_2.os = 1;%NFFT/hop;
scparam_2.Q = 32;
scparam_2.N=Npad_2;
filts_2 = cqt_prepare(scparam_2);

pp.scparam = scparam_2;
pp.filts = filts_2;
...
xo=pad_mirror(x,Npad);         
Xt1 = batchscatt(xo',filts, scparam);  
%V2 = abs(Xt2);
V1 = softNormalize(abs(Xt1),epsilon);
[F,N] = size(V1);
...
xo=pad_mirror(x,Npad);         
Xt2 = batchscatt(xo',filts, scparam);  
%V2 = abs(Xt2);
V2 = softNormalize(abs(Xt2),epsilon);
[F,N] = size(V2);
...
[X, phmix] = batchscatt(pad_mirror(mix',Npad_2), filts_2, scparam_2);
V = softNormalize(abs(X),epsilon);
[F,N] = size(V);
...
eps_1 = 1e-6;
p1 = 1;
V_ap = W1H1.^p1 +W2H2.^p1 + eps_1;

% wiener filter
SPEECH1 = ((W1H1.^p1)./V_ap).*X;
SPEECH2 = ((W2H2.^p1)./V_ap).*X;
% speech1 = invert_spectrum(SPEECH1,l_win,overlap,T);
% speech2 = invert_spectrum(SPEECH2,l_win,overlap,T);
