...
for i=1:n
  ...
  switch l.type
    case 'nmf'
      res(i+1).x = vl_nmf(res(i).x, l.D1, l.D2, l.lambda,[]) ;
    case 'sc'
      if ~isfield(l,'pos'), l.pos = 0;end
      if ~isfield(l,'output'), l.output = 'Y';end
      res(i+1).x = vl_sc(res(i).x, l.dict, l.lambda,[],'pos',l.pos,'output', l.output) ;
    case 'conv'
      res(i+1).x = vl_nnconv(res(i).x, l.filters, l.biases, 'pad', l.pad, 'stride', l.stride) ;
      if isnan(sum(res(i).x(:)))
          keyboard
      end
      if isnan(sum(res(i+1).x(:)))
          keyboard
      end
      case 'pool'
    ...
    case 'channelPoole'
      res(i+1).x = vl_channelPoole(res(i).x,l.nchannels);
    case 'filtermask'
    ...
    case 'fitting'
      res(i+1).x = vl_fit(res(i).x,l.Ymix,l.Y1,l.Y2,[],'loss',l.loss);
    case 'relu'
...
if doder
  ...
  for i=n:-1:1
    ...
    switch l.type
        case 'nmf'
            [res(i).dzdx, res(i).dzdw]  = vl_nmf(res(i).x, l.D1, l.D2, l.lambda,res(i+1).dzdx) ;
        case 'sc'
            if ~isfield(l,'pos'), l.pos = 0;end
            if ~isfield(l,'output'), l.output = 'Y';end
            [res(i).dzdx, res(i).dzdw{1}] = vl_sc(res(i).x, l.dict, l.lambda,res(i+1).dzdx,'pos',l.pos,'output', l.output) ;
        case 'conv'
      ...
      case 'channelPoole'
            res(i).dzdx = vl_channelPoole(res(i).x,l.nchannels,res(i+1).dzdx);
      case 'filtermask'
      ...
      case 'softmaxloss'
        ...
        case 'fitting'
         res(i).dzdx = vl_fit(res(i).x,l.Ymix,l.Y1,l.Y2,res(i+1).dzdx,'loss',l.loss);
        case 'relu'
