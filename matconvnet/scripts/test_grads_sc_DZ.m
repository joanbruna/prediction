...
DZ = vl_sc(X,D,lambda,[],'single',0,'output','DY');


Z0 = rand(size(DZ));

obj = 0.5*norm(DZ(:)-Z0(:),'fro')^2;
dzdy = DZ-Z0;

[dzdx,dzdw] = vl_sc(X,D,lambda,dzdy,'single',0,'output','DY');
...
DZ_ = vl_sc(X_,D,lambda,[],'single',0,'output','DY');

%y = vl_nnconv(x,w,b,'verbose') ;


obj_ = 0.5*norm(DZ_(:)-Z0(:),'fro')^2;
...
Z_ = vl_sc(X,D_,lambda,[],'single',0,'output','DY');
...
net_sc.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(1,1,size(D,1),10, 'single'),...
